...
Destination Unreachable messages. On the unreachable errors generated
when an admitted packet cannot be forwarded because the required link
is presently unusable, the objects convey the expected time until the
link becomes usable, the time the estimate was generated, and the
expected one-way delay of the link. On the administrative prohibition
errors generated when the link's admission policy refuses a packet, a
presence-only object identifies boundary-link admission policy as the
source of the refusal. The underlying codes retain their existing
meanings, and this document requests no new ICMP type or code. It
specifies when the objects may be attached and what they convey, not
how transports, applications, or hosts react to them; making the
...
extension objects {{RFC4884}} that let a gateway in such a domain
supply additional information on those messages. On the unreachable
errors generated when an admitted packet cannot be forwarded because
the required link is presently unusable, the gateway may attach the
time at which it generated the message, its estimate of the time until
the link becomes usable, and the approximate one-way delay of the
link. On the administrative prohibition errors generated when the
link's admission policy refuses a packet, it may attach a
presence-only object indicating that boundary-link admission policy,
rather than some other administrative control, was the source of the
refusal. The underlying code retains its existing meaning in every
case ({{common}}).
...
publishing the objects is to enable experimentation with them
({{consumers}}, {{experiment}}). The availability metadata is useful
because the boundary is close and the condition is long-lived: gateway
feedback reaches senders in local round-trip times, while the
unavailability it reports lasts minutes to hours ({{applicability}}).
The policy indication is useful for a different reason: it tells an
application that changing what it sends, rather than waiting, is what
would help.
...
The objects are specified for use under a single administrative
authority or cooperating authorities sharing the coordination and
disclosure scopes defined below. {{RFC8799}} provides broader context
on limited-domain deployments.

Three relationships need to be kept distinct.

The first is the timing relationship between a notifying gateway and
the senders it notifies. Let R be the round-trip time between a sender
and the gateway, D the duration of a link unavailability being
reported, and T the timescale on which the sender's transport would
otherwise detect failure. Availability metadata is useful when R is
small relative to T, so the notification arrives while the sender
still has options, and small relative to D, so the reported condition
is still in effect on arrival. In the Mars enclave of {{conops}}, R is
milliseconds to seconds, T is seconds to minutes, and D is minutes to
hours. When R approaches D, a notification describes a link state that
may no longer exist on arrival; gateways therefore attach objects only
into their local enclave and never across the constrained long-haul
link. This timing argument is specific to availability metadata; a
policy refusal applies regardless of current link usability and needs
no predicted outage to be worth reporting.
...
scope, not about every path that could ever exist, unprovisioned
routes, unrelated domains, or application-layer alternatives.
Equally, the failure of one local link is not a basis for an assertion
when the current managed forwarding state provides a usable,
policy-permitted alternative for that traffic.
...
Cooperating domains must agree on both scopes; their agreement does
not authenticate ICMP ({{security}}), and a short local round-trip
time does not by itself make a network part of either scope. For
comparison of timescales only, the public Internet has sub-second
internal round-trip times relative to a link whose unavailability is
measured in minutes, but it is not thereby a cooperating or trusted
signaling domain.
...
the link, never on the basis of observed local carrier or interface
state alone; a router that merely notices its outgoing interface is
down generates whatever error ordinary ICMP rules call for and has no
basis to attach Link Condition Objects to it. How the domain acquires
and distributes its knowledge is not specified; the objects encode
what the gateway is entitled to assert, not how it learned it. The
same criteria identify deployments beyond the interplanetary case,
...
and high delay alone is not a reason to generate a Destination
...
traffic toward whichever gateway and link can serve it. Routing, link,
and policy updates are not installed atomically, however, and
distribution itself crosses the deep-space link; during transitions,
or when plan and reality diverge, a gateway may receive traffic that
another gateway could have served. Gateways operate autonomously
between management contacts on their most recent applicable knowledge
and omit assertions they cannot currently support ({{generation}}).

Intra-enclave traffic is unaffected; hosts that never originate
trans-boundary traffic need not implement the objects.
...
one-way delay over the link at that time as 20 minutes. It installs
or schedules the routing and admission policy needed for that link,
and it supplies the gateway with an associated record containing at
least:
...
no-route error. Any absolute time the controller supplies must either
be expressed in, or come with a known conversion into, the gateway's
enclave time reference ({{gen-time}}); alternatively the controller
may supply a duration anchored to a reference time the gateway can
interpret. A readiness time the gateway cannot relate to its own
reference is not a basis for ETU.
...
application that the path is deferred rather than failed
({{consumers}}).
...
communication options. A DTN-aware host might hand the affected
traffic to its local BP agent, which originates bundles toward the
destination for forwarding during the next contact. An application
with separately provisioned knowledge of an authorized
application-layer gateway might direct its traffic there. An
application with its own delay tolerance might instead compare the
expected wait against that tolerance and fail fast.
...
traffic was refused or what traffic would be admitted: admission
...
obtaining admission is a provisioning action. The application may
reconsider its communication mechanism ({{uc-handoff}}).
...
evaluated first ({{generation}}). Traffic that would be refused
regardless of link state receives the administrative prohibition
error, optionally with the Denied by Link Policy object, and never
receives availability metadata, which would disclose link schedule
information to traffic that could not use the link anyway. Traffic
that is admitted but whose link is not presently usable receives the
ordinary unreachable error, optionally with availability metadata. A
sender whose denial is resolved through provisioning may then receive
a metadata-bearing unreachable error for the same flow: first become
admissible, then wait for the link.
...
has previously used them experimentally as indications of
connectivity disruption. Nor does an ordinary administrative
prohibition error identify which administrative control refused the
packet. Availability metadata adds to an unreachable error the
domain's assertion that the failure reflects a managed transient
condition of a constrained long-haul link to which the traffic was
admitted, and, where supplied, an estimate of its duration. The Denied
by Link Policy object adds to an administrative prohibition error the
assertion that boundary-link admission policy was the source of the
refusal.
...
The objects add to a Destination Unreachable message; they never
change what its code means. A message that carries no Link Condition
Objects, whether because the gateway attached none or because the
receiver ignores or does not process extensions, is an ordinary
Destination Unreachable message and means only what its code has
always meant. Absence of an object supplies no inference: it does not
indicate that admission succeeded or failed, that a constrained
long-haul link was or was not involved, or that any prediction exists.
A message carrying no objects defined here need not carry an extension
structure at all, and an extension structure, when present, may also
carry other independently applicable {{RFC4884}} objects. Recognition
of a carrier code implies nothing about extension support; the
extension structure lies beyond the portion of the original datagram
that extension-unaware receivers examine.
...
When a gateway does not forward an otherwise admitted packet because
the constrained long-haul link required to reach its destination is
presently unusable, it generates whatever Destination Unreachable
error the ordinary rules of {{RFC1812}} or {{RFC4443}} call for,
according to its own forwarding state. This document does not change
which code is generated or when. It permits the gateway to attach
availability metadata -- the Generation Time, Expected Time Until Link
Usability, and Expected Link Delay objects -- to that error, subject
...
forwarding state for the affected traffic within the coordination
scope ({{applicability}}). The transient interruption may coincide
with absence of an installed route: a domain may withdraw the route
to a destination between contacts while retaining the managed-service
knowledge that explains the failure, and the resulting no-route error
is an eligible carrier. A gateway that meets these conditions but has
no estimate to supply, or withholds estimates by policy, sends the
ordinary error without the objects; the gateway need not know when
the condition will clear.

These assertions are carried by the Expected Time Until Link Usability
and Expected Link Delay objects and attach to nothing else
({{common}}). A Generation Time object supplies a timestamp and is not
...
within that set, each object is optional and absence means only that
the gateway did not supply that information ({{common}}). No sentinel
values are defined for absent data. Because the class includes both
availability metadata and a policy indication, each assertion in this
document is attributed to the specific object that carries it.
...
({{uc-denied}}). An error conveys this classification only when it
contains a valid, applicable Denied by Link Policy object; an object
of any other length does not establish it ({{common}}), and absence
of the object does not establish that the refusal came from some
other control, that no boundary-link policy exists, or that the
application has no alternatives.

Generating the object is optional and subject to disclosure policy
({{generation}}). Its presence does not promise permanent denial,
prove that waiting could never help, require the receiver to change
...
A policy carrier MUST NOT carry Expected Time Until Link Usability or
Expected Link Delay: they reveal characteristics of a link to which
the invoking traffic was not admitted, and admission is evaluated
before any link state is disclosed ({{generation}}). An availability
carrier MUST NOT carry Denied by Link Policy, since an admitted packet
was by definition not refused by admission policy. Under these rules
Denied by Link Policy and the availability metadata are never emitted
in the same message.

Generation Time SHOULD NOT be sent without Expected Time Until Link
Usability on either carrier group, since this document defines no
other use for it and unnecessary absolute-time information should not
normally be disclosed. This is discouraged rather than inapplicable or
prohibited: a receiver that finds Generation Time without ETU simply
has no defined use for it, and experimentation or later experience
may identify one.

These are sender restrictions; receiver tolerance for inapplicable
objects is specified in {{common}} and does not relax them.
...
3. If the traffic is not admitted, drop it. Where ordinary rules and
   ...
   and nothing else ({{applicability-objects}}). Denial of admission
   to one long-haul service does not by itself mean no policy-permitted
...
The restrictions in this section govern attaching Link Condition
Objects; they do not alter when ordinary Destination Unreachable
errors are generated. A gateway MUST NOT attach any Link Condition
Object whose assertion it cannot establish from a sufficiently current
and applicable association between the traffic, the managed service,
the forwarding state, the admission policy, and any advertised
prediction ({{orchestration}}). In particular, a gateway MUST NOT
attach availability metadata when another policy-permitted path within
the coordination scope is presently available for the traffic, and
MUST NOT attach it on the basis of observed local interface or carrier
state alone ({{applicability}}). Whenever an object's basis is
missing, the object is withheld by policy, or an estimate is out of
range ({{durations}}), the gateway omits that object and sends the
ordinary error.

A gateway MUST NOT send an ETU value not derived from its current
managed-service knowledge, and MUST NOT report an expired estimate
clamped to zero. A gateway MAY send ETU without Generation Time; it
SHOULD NOT send Generation Time without ETU
({{applicability-objects}}).
...
it. An operator SHOULD be able to configure omission of the
...
ICMP delivery is unreliable and rate limited. A gateway MUST NOT
assume that a sender received any particular notification; it
generates a fresh notification, subject to rate limiting, for each
invoking packet that meets the conditions above, using its current
...
document does not change them, and the checks below apply only to the
objects. Disregarding an object never requires rejecting, or changing
the ordinary handling of, the base error.
...
selectively remove Link Condition Objects without also affecting
ordinary errors of the same code. A deployment that requires selective
containment of the objects at a boundary needs extension-aware
enforcement; this document does not specify one and does not mandate
filtering of the carrier codes.
...
refused the traffic. What follows are possibilities that the objects
...
Unreachable error, optionally carrying additional information, so a
stack that does not process {{RFC4884}} extensions handles it exactly
as it handles the same error today ({{common}}). This document does
not promise identical behavior across stacks or that any connection
survives.
...
the basis on which this document selects carriers, do not define
permanent versus temporary failure, and this document neither relies
on nor promises any particular transport reaction.
...
of the carrier codes alone do not identify this experiment's activity,
since those codes are generated for other reasons as well. A gateway
attaching Denied by Link Policy knows which policy dimension refused
the traffic even though the object does not convey it, and MAY log or
report that detail through local operational telemetry, which is where
operators debugging a refused flow should look. This document does not
...
are deliberately not standardized here ({{consumers}}); the experiment
is intended to produce the experience on which later specification of
such behaviors might be based. An extension object, like a new code,
requires implementation support to expose a new meaning to
applications; neither inherently solves application delivery.

## Scope of the Experiment {#exp-scope}

The experiment runs within domains meeting the criteria of
{{applicability}}: space networking testbeds and missions, other
networks with scheduled or intermittently available managed links
between enclaves, and laboratory emulations of the same topologies.
Link Condition Objects are not intended to be exchanged across
uncontrolled networks ({{validation}}). No behavior in this document
affects hosts whose traffic never requires a trans-boundary link.
...
authorities. None of them authenticates the carrier code or the
objects; filtering and invoking-packet validation raise the cost of
injection without confining it to trusted parties, and none is
dependable across the open internet.
...
Because the objects ride on ordinary codes, containing them at a
boundary requires extension-aware enforcement ({{validation}}). The
absence of Link Condition Objects from a message is not a trustworthy
indication that no long-haul condition exists ({{common}}), since
...
Had the gateway attached no objects, or had the receiver not
processed them, the message would be an ordinary Address Unreachable
error ({{common}}).
...
accuracy of the forecast. Had the gateway instead generated a
notification at 10,003 s against the same forecast, that notification
would carry Generation Time 10,003 s and ETU 2,517,000 ms: ETU is
recomputed for each notification from the current reference time, not
...
traffic once the link is usable ({{consumers}}).
...
that ignores extensions, or that disregards the object, has an
ordinary administrative prohibition error and nothing more
({{common}}).
